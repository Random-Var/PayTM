...
app.post("/webhook/v1/onRamp", async (req, res) => {
    ...
    try{
        const amount = await db.hDFCBank.findFirst({
            where: {
...
app.post("/webhook/v1/offRamp", async (req, res) => {
    ...
    try{
        const request = await db.hDFCBank.findFirst({
            where: {
                userId: Number(paymentInformation.userId),
                token: paymentInformation.token,
                status: "Processing",
...
app.listen(PORT, () => {
    console.log(`HDFC Bank webhook running on port ${PORT}`);
})
